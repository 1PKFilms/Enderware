...
public class Block
{
    ...
    {
        Event event = new setBlockByPlayerEvent(player.dimension, x, y, z,  this.blockID,world.getBlockMetadata(x, y, z), 0,0, player);
        if(MinecraftForge.EVENT_BUS.post(event))return false;
        if(event.getResult().compareTo(Result.DENY)== 0)return false;
        return world.setBlockAndMetadataWithoutEventAndWithNotify(x, y, z, 0, 0);
    }
